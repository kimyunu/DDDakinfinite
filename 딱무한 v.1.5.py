...
def set_position():
    try:
        script_path = resource_path("point_selector.py")

        # 파이썬 경로 자동 감지
        if hasattr(sys, "_MEIPASS"):
            python_exec = os.path.join(sys._MEIPASS, "python.exe")
        else:
            python_exec = "python"
        ...
        if len(coords) == 2:
            ...
            entry_x.insert(0, coords[0])
            entry_y.delete(0, tk.END)
            entry_y.insert(0, coords[1])
    except Exception as e:
        print("위치 지정 중 오류:", e)
...
btn_pos = tk.Button(root, text="딱 위치 고르기", command=set_position)
